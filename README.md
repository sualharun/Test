...
**Branch `vuln-llm07-eval-exec`:** adds `POST /vuln/llm07/agent-eval` (`eval` sink). Conflicts with other `vuln-*` branches on `demo/vuln-merge-token.txt` and this line when merging without rebase.
