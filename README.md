...
**Branch `vuln-arbitrary-file-upload`:** adds `POST /vuln/upload/insecure`. Conflicts on `frontend/package.json` version with `vuln-shell-command-injection`, and on shared merge token/README with other `vuln-*` branches.
