...
**Branch `vuln-environment-secrets-leak`:** adds `GET /vuln/debug/environment` returning `os.environ` plus signing secrets. Collides with other `vuln-*` PRs on `taskboard/main.py` router wiring, `demo/vuln-merge-token.txt`, and this README line.
